...
$stmt->execute([$id]);
$item = $stmt->fetch();
if (!$item) die('존재하지 않는 공지입니다');

// 추가 이미지 가져오기
$img_stmt = $pdo->prepare("SELECT * FROM news_images WHERE news_id = ? ORDER BY sort_order ASC");
$img_stmt->execute([$id]);
...
<main class="container" style="padding-top:clamp(70px,12vh,120px); max-width:720px;">
  <article class="news-detail">
    <h2 class="section-title"><?= htmlspecialchars($item['title']) ?></h2>
    <p class="card-meta">작성일: <?= substr($item['created_at'], 0, 10) ?></p>
    
    <!-- 대표 이미지 -->
    <?php if (!empty($item['featured_image'])): ?>
      <img src="../img/news/<?= htmlspecialchars($item['featured_image']) ?>" 
      ...
      <div class="gallery">
        <?php foreach ($images as $index => $image): ?>
          <div class="gallery-item" onclick="openModal();currentSlide(<?= $index + 1 ?>)">
            <img src="../img/news/<?= htmlspecialchars($image['image_path']) ?>" 
                 alt="소식 이미지 <?= $index + 1 ?>">
          </div>
        <?php endforeach; ?>
      </div>
    ...
    <div style="text-align: center; margin-top: 3rem;">
      <a href="news.php" class="btn">목록으로 돌아가기</a>
    </div>
  </article>
</main>
