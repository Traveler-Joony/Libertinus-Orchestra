...
if (isset($_GET['delete'])) {
    ...
    $img_stmt = $conn->prepare("SELECT featured_image FROM performances WHERE id = ?");
    ...
    // 추가 이미지들 정보 가져오기
    $gallery_stmt = $conn->prepare("SELECT image_path FROM performance_images WHERE performance_id = ?");
    ...
    // 공연 및 연결된 이미지 레코드 삭제 (외래키 제약조건으로 자동 삭제됨)
    $conn->query("DELETE FROM performances WHERE id = $id");
    
    // 물리적 이미지 파일 삭제
    if (!empty($img_data['featured_image'])) {
        $featured_path = "./img/performance/" . $img_data['featured_image'];
    ...
    foreach ($gallery_images as $image) {
        $gallery_path = "./img/performance/" . $image;
...
if (isset($_GET['delete_image'])) {
    ...
    // 이미지 파일 경로 가져오기
    $img_stmt = $conn->prepare("SELECT image_path FROM performance_images WHERE id = ?");
    ...
    // 이미지 레코드 삭제
    $del_stmt = $conn->prepare("DELETE FROM performance_images WHERE id = ?");
    $del_stmt->bind_param("i", $image_id);
    $del_stmt->execute();
    
    // 물리적 이미지 파일 삭제
    if (!empty($img_data['image_path'])) {
        $image_path = "./img/performance/" . $img_data['image_path'];
        if (file_exists($image_path)) {
            @unlink($image_path);
        }
    }
    
    header("Location: admin_performances.php?edit=" . $performance_id);
    exit;
}
...
if ($edit_id) {
    ...
    // 공연 기본 정보 가져오기
    $stmt = $conn->prepare("SELECT * FROM performances WHERE id = ?");
    ...
    // 공연 추가 이미지 가져오기
    $img_stmt = $conn->prepare("SELECT * FROM performance_images WHERE performance_id = ? ORDER BY sort_order ASC");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($_FILES["featured_image"]["name"])) {
        $target_dir = "./img/performance/"; // 이미지 저장 경로
    ...
    if (!$error) {
        ...
        try {
            if (!empty($_POST['edit_id'])) {
                ...
                $stmt = $conn->prepare("UPDATE performances SET title=?, content=?, featured_image=?, youtube_url=?, main_display=?, display=? WHERE id=?");
                $stmt->bind_param("ssssiis", $title, $content, $filename, $youtube_url, $main_display, $display, $id);
                $stmt->execute();
                $performance_id = $id;
            } else {
                // 새 글 등록
                $stmt = $conn->prepare("INSERT INTO performances (title, content, featured_image, youtube_url, main_display, display, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())");
                $stmt->bind_param("ssssii", $title, $content, $filename, $youtube_url, $main_display, $display);
                $stmt->execute();
                $performance_id = $conn->insert_id;
            }
            
            // 갤러리 이미지 업로드 처리
            if (!empty($_FILES['gallery_images']['name'][0])) {
                $gallery_dir = "./img/performance/"; // 갤러리 이미지 경로 수정
                ...
                $order_stmt = $conn->prepare("SELECT MAX(sort_order) as max_order FROM performance_images WHERE performance_id = ?");
                $order_stmt->bind_param("i", $performance_id);
                $order_stmt->execute();
                ...
                for ($i = 0; $i < $file_count; $i++) {
                    if ($_FILES['gallery_images']['error'][$i] === UPLOAD_ERR_OK) {
                        ...
                        if (move_uploaded_file($temp_file, $gallery_path)) {
                            ...
                            $img_stmt = $conn->prepare("INSERT INTO performance_images (performance_id, image_path, sort_order, created_at) VALUES (?, ?, ?, NOW())");
                            $img_stmt->bind_param("isi", $performance_id, $gallery_filename, $sort_order);
                            $img_stmt->execute();
                        }
                    }
                }
...
// 공연 목록 조회
$performance_list = $conn->query("SELECT * FROM performances ORDER BY created_at DESC");
?>

<style>
.admin-flex { display: flex; gap: 2rem; align-items: flex-start; flex-wrap: wrap; }
.admin-left, .admin-right { flex: 1 1 400px; }
.performance-table { width: 100%; border-collapse: collapse; font-size: 0.95rem; }
.performance-table th, .performance-table td { border: 1px solid #ccc; padding: 0.6rem; text-align: left; }
.performance-table th { background: #f1f3f5; }
...
<main class="join-wrap" style="padding-top: 150px !important;">
  <div class="container admin-flex">
    ...
    <div class="join-box admin-left">
      <h2><?= $edit_mode ? "✏️ 공연 수정" : "🎭 공연 등록" ?></h2>

      <?php if ($success): ?>
        <p class="form-success">✅ 공연이 등록되었습니다.</p>
      <?php elseif ($error): ?>
      ...
      <form method="post" enctype="multipart/form-data">
        ...
        <?php if (!empty($edit_data['featured_image'])): ?>
          ...
          <img src="./img/performance/<?= htmlspecialchars($edit_data['featured_image']) ?>" 
          ...
          <div class="gallery-preview">
            <?php foreach ($gallery_images as $image): ?>
              <div class="gallery-item">
                <img src="./img/performance/<?= htmlspecialchars($image['image_path']) ?>" alt="공연 이미지">
                <a href="?delete_image=<?= $image['id'] ?>&performance_id=<?= $edit_id ?>" 
                   class="delete-image" 
    ...
    <div class="join-box admin-right">
      <div style="display:flex;justify-content:space-between;align-items:center;">
        <h2>📋 공연 목록</h2>
        <a href="admin_performances.php" class="btn btn-sm" style="font-size:0.88rem;">➕ 새 공연 등록</a>
      </div>
      <table class="performance-table">
        ...
        <tbody>
          <?php while ($row = $performance_list->fetch_assoc()): ?>
            <tr>
              <td>
